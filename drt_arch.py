...
from traiNNer.archs import drft_arch as _drft
from traiNNer.archs.drft_arch import (
    ATTN_TYPE,
    ACTBlock,
    AttentionBlocks,
    DRFT,
...
def _selective_drt_qk_stats_policy(
    ctx: Any,
    op: Any,
    *args: Any,
    **kwargs: Any,
) -> CheckpointPolicy:
    """Extend the canonical policy with DRT's per-head Q/K RMS statistic."""

    base_policy = _drft._selective_flash_qkv_norm_stats_policy(
        ctx,
        op,
        *args,
        **kwargs,
    ...
    if op is torch.ops.aten.mean.dim and len(args) >= 2:
        ...
        if (
            ...
            and source.ndim == 4
            and dimensions in ((-1,), (3,))
        ):
            return CheckpointPolicy.MUST_SAVE
    return CheckpointPolicy.PREFER_RECOMPUTE


_DRT_QK_STATS_SELECTIVE_CHECKPOINT_CONTEXT = partial(
    create_selective_checkpoint_contexts,
    _selective_drt_qk_stats_policy,
)
        ...
        if self.training and device.type == "cuda":
            for module in self.modules():
                if isinstance(module, AttentionBlocks):
                    ...
                    module.compiled_checkpoint_context_fn = (
                        _DRT_QK_STATS_SELECTIVE_CHECKPOINT_CONTEXT
                    )
...
def drt_xl(
    ...
    **kwargs: Any,
    ...
    """DRT XL: C256/G14, retaining the original deep XL geometry."""
...
try:
    ...
    for factory in (
        ...
        ARCH_REGISTRY.register(factory)
except ImportError:
    pass
